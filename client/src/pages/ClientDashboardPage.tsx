                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-3">
                    ...
                    <Select 
                      value={(() => {
                        ...
                        return model?.categoryId?.toString() || "none";
                      })()} 
                      onValueChange={(value) => {
                        const categoryId = value === "none" ? null : parseInt(value);
                        updateCategoryMutation.mutate({ modelId: expandedModelId, categoryId });
                      }}
                      disabled={updateCategoryMutation.isPending}
                    >
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-2">
                      <Select 
                        value={selectedLanguages[expandedModelId] || currentLanguage} 
                        onValueChange={(value) => {
                          setSelectedLanguages(prev => ({ ...prev, [expandedModelId]: value }));
                        }}
                      >
                        <SelectTrigger className="w-full">
                      ...
                      <Textarea
                        ...
                        value={modelDescriptions[`${expandedModelId}_${selectedLanguages[expandedModelId] || currentLanguage}`] || ""}
                        onChange={(e) => {
                          const key = `${expandedModelId}_${selectedLanguages[expandedModelId] || currentLanguage}`;
                      ...
                      <Button 
                        size="sm"
                        onClick={() => {
                          const language = selectedLanguages[expandedModelId] || currentLanguage;
                          const description = modelDescriptions[`${expandedModelId}_${language}`];
                          if (description?.trim()) {
                            console.log(`Saving description for model ${expandedModelId} in ${language}: ${description}`);
                            toast({
                              title: t('success'),
