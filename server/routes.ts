...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Endpoint do administratorskiego dostępu do chronionego pliku bez hasła
  app.post("/api/admin/shared-models/:id/access", async (req: Request, res: Response) => {
    try {
      const { id } = req.params;
      const modelId = parseInt(id);
      ...
      // Sprawdź token administratora (w rzeczywistej aplikacji powinna być dokładniejsza weryfikacja)
      ...
      // Jeśli model nie jest udostępniony lub nie ma shareId
      if (!model.shareEnabled || !model.shareId) {
        return res.status(400).json({ message: "Model is not shared" });
      }
      ...
      // Administrator ma dostęp bez hasła - zapisz statystyki dostępu
      const ipAddress = req.ip || 'admin-access';
      const userAgent = req.headers['user-agent'] || 'admin-client';
      ...
      try {
        ...
        console.error("Error recording admin model view:", viewError);
        // Kontynuujemy nawet jeśli zapis statystyk nie powiedzie się
      }
      ...
      // Zwróć podstawowe dane modelu dla administratora
      res.json({
        id: model.id,
        filename: model.filename,
        filesize: model.filesize,
        ...
        created: model.created,
        sourceSystem: model.sourceSystem,
        adminAccess: true // Oznaczenie, że dostęp uzyskano przez administratora
      });
    } catch (error) {
      console.error("Error in admin model access:", error);
      res.status(500).json({ message: "Failed to access model as admin" });
    }
  });
